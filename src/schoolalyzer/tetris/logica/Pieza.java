...
public class Pieza {
    ...
    protected final int VACIO = schoolalyzer.tetris.configuracion.Otros.getNumeroVacio(); //Numero que nos marca un vacio (no hay pieza).
    ...
    protected boolean calcularColision(Tablero tablero) {
        ...
        if (!this.cabePieza(tablero)) {
            ...
            colision = true;
        } //<-- y si se mueve a un lado antes de que baje??? bueno, el cicloJuego() ya permite mover una vez antes de solidificarse (y vuelve a comprobar la colision, por si se ha movido).
    ...
    public boolean rotarIzquierda(Tablero tablero, boolean desplazarPiezaSiNoCabeAlRotar) {
        ...
        for (byte f = 0; f < this.getAlto(); f++) {
            for (byte c = 0; c < this.getAncho(); c++) {
                ...
                if (esIgual && formaRotada[fRotada][cRotada] != this.getForma()[f][c]) {
                    esIgual = false;
                }
            }
        }
        ...
        if (!this.cabePieza(piezaRotada, tablero) && desplazarPiezaSiNoCabeAlRotar) {
            ...
            } //Si es mas larga que ancha, la desplaza a la izquierda y arriba.
            else {
                piezaRotada.desplazarDerechaArriba(tablero);
            } //Si es mas ancha que larga o igual (cuadrada, aunque tenga vacios), la desplaza a la derecha y arriba.
        }
    ...
    public boolean rotarDerecha(Tablero tablero, boolean desplazarPiezaSiNoCabeAlRotar) {
        ...
        if (!this.cabePieza(piezaRotada, tablero) && desplazarPiezaSiNoCabeAlRotar) {
            ...
            if (piezaRotada.getTop() > piezaRotada.getLeft()) {
                piezaRotada.desplazarDerechaArriba(tablero);
            } //Si es mas larga que ancha, la desplaza a la derecha y arriba.
            else {
                piezaRotada.desplazarIzquierdaArriba(tablero);
            } //Si es mas ancha que larga o igual (cuadrada, aunque tenga vacios), la desplaza a la izquierda y arriba.
        }
