...
import schoolalyzer.tetris.configuracion.*; //Utiliza el archivo de configuracion del juego.
...
import schoolalyzer.tetris.gui.MotorAnimaciones;
import schoolalyzer.tetris.sonido.MotorSonido;

public class TetrisGame {

    protected int[] teclaArriba = schoolalyzer.tetris.configuracion.Teclas.getTeclaArriba(); //Tecla(s) para ir arriba (en Tetris no se puede, pero por si hubiera algun menu).
    protected int[] teclaAbajo = schoolalyzer.tetris.configuracion.Teclas.getTeclaAbajo(); //Tecla(s) para ir abajo.
    protected int[] teclaDerecha = schoolalyzer.tetris.configuracion.Teclas.getTeclaDerecha(); //Tecla(s) para ir a la derecha.
    protected int[] teclaIzquierda = schoolalyzer.tetris.configuracion.Teclas.getTeclaIzquierda(); //Tecla(s) para ir a la izquierda.
    protected int[] teclaRotarDerecha = schoolalyzer.tetris.configuracion.Teclas.getTeclaRotarDerecha(); //Tecla(s) para rotar a la derecha.
    protected int[] teclaRotarIzquierda = schoolalyzer.tetris.configuracion.Teclas.getTeclaRotarIzquierda(); //Tecla(s) para rotar a la derecha.
    protected int[] teclaAceptar = schoolalyzer.tetris.configuracion.Teclas.getTeclaAceptar(); //Tecla(s) para aceptar.
    protected int[] teclaPausa = schoolalyzer.tetris.configuracion.Teclas.getTeclaPausa(); //Tecla(s) para pausar.
    protected int[] teclaSalir = schoolalyzer.tetris.configuracion.Teclas.getTeclaSalir(); //Tecla(s) para salir.
    ...
    protected int velocidadLectorTeclado = schoolalyzer.tetris.configuracion.Otros.getVelocidadLectorTeclado(); //Milisegundos que tarda en volver a leer el teclado.
    protected int retrasoTeclasRotacion = schoolalyzer.tetris.configuracion.Otros.getRetrasoTeclasRotacion(); //Milisegundos como minimo entre dos procesamientos de tecla, cuando es de rotacion.
    protected int retrasoTeclasOtras = schoolalyzer.tetris.configuracion.Otros.getRetrasoTeclasOtras(); //Milisegundos como minimo entre dos procesamientos de tecla (todas menos de rotacion).
    ...
    protected boolean sonidoActivado = schoolalyzer.tetris.configuracion.Otros.getSonidoActivado(); //Define si se reproducen sonidos o no.
    protected boolean musicaActivada = schoolalyzer.tetris.configuracion.Otros.getMusicaActivada(); //Define si se reproducen musica o no.
    ...
    protected int velocidad = schoolalyzer.tetris.configuracion.Otros.getVelocidadInicial(); //Retraso entre ciclos, en milisegundos (numero menor = mas rapido).
    protected int velocidadMinima = schoolalyzer.tetris.configuracion.Otros.getVelocidadMinima(); //Velocidad minima posible (numero menor = mas rapido).
    protected int velocidadMaxima = schoolalyzer.tetris.configuracion.Otros.getVelocidadMaxima(); //Velocidad maxima posible (numero menor = mas rapido).
    protected int incrementoVelocidad = schoolalyzer.tetris.configuracion.Otros.getIncrementoVelocidad(); //Incremento de la velocidad cuando esta sube (si es negativo, la velocidad sube).
    ...
    protected int puntosPorPiezaPuesta = schoolalyzer.tetris.configuracion.Otros.getPuntosPorPiezaPuesta();
    protected int puntosPorLineaAdicional = schoolalyzer.tetris.configuracion.Otros.getPuntosPorLineaAdicional();
    protected int puntosPorLinea = schoolalyzer.tetris.configuracion.Otros.getPuntosPorLinea();
    protected int lineas; //Lineas hechas.
    protected int lineasNivel; //Lineas hechas en el nivel actual.
    protected int lineasNecesariasNivel = schoolalyzer.tetris.configuracion.Otros.getLineasNecesariasNivel(); //Lineas necesarias para pasar de nivel.
    ...
    public TetrisGame(Graphics contenedor) {
        this(contenedor, null, null);
    }

    //Constructor que recibe el contenedor y la paleta del juego:
    public TetrisGame(Graphics contenedor, Paleta paleta) {
    ...
    public TetrisGame(Graphics contenedor, Paleta paleta, Pieza[] piezasBase) {
        this.setContenedorGrafico(contenedor);

        //Si no se ha enviado una paleta, pone la definida en la configuracion:
        if (paleta == null) {
            paleta = schoolalyzer.tetris.configuracion.PaletaColores.getPaleta();
        }
        this.setPaleta(paleta);
        //Si no se ha enviado piezas base, pone las definidas en la configuracion:
        if (piezasBase == null) {
            piezasBase = schoolalyzer.tetris.configuracion.Piezas.getPiezas(this.getPaleta());
        }
        this.setPiezasBase(piezasBase);

        //Crea el tablero con el alto y ancho definidos en la configuracion:
        byte alto = schoolalyzer.tetris.configuracion.Otros.getTableroAlto();
        byte ancho = schoolalyzer.tetris.configuracion.Otros.getTableroAncho();
        ...
        this.menuPrincipal = schoolalyzer.tetris.configuracion.Menus.getMenuPrincipal();
        this.menuPrincipal.setPaleta(this.getPaleta());

        //Instancia el menu de pausa:
        this.menuPausa = schoolalyzer.tetris.configuracion.Menus.getMenuPausa();
        this.menuPausa.setPaleta(this.getPaleta());

        //Carga los sonidos:
        if (!MotorSonido.sonidosCargados) {
    ...
    public void setPiezasBase(Pieza[] piezasBase) {
        ...
        if (this.getPanel() != null) {
            ...
            TetrisMain.resizeMainWindow(); //Quiza se deba comentar esto si se utiliza un Applet.
    ...
    public void redimensionarImagenBuffer() {
        ...
        if (schoolalyzer.tetris.configuracion.Otros.getTipoPrograma() == 0) {
            imagenAncho = TetrisMain.ventanaPrincipal.getWidth();
            imagenAlto = TetrisMain.ventanaPrincipal.getHeight();
        } else if (schoolalyzer.tetris.configuracion.Otros.getTipoPrograma() == 1) {
            imagenAncho = TetrisMain.appletPrincipal.getWidth();
            imagenAlto = TetrisMain.appletPrincipal.getHeight();
        }
        ...
        if (this.teclaEnTeclas(tecla, this.getTeclaAceptar())) {
            ...
            if (this.getMenuPrincipal() != null) {
                if (this.getMenuPrincipal().getMostrar()) {
                    ...
                    if (this.getSonidoActivado()) {
                        MotorSonido.menuOpcionAceptada();
                    }
        ...
        if (this.teclaEnTeclas(tecla, this.getTeclaArriba())) {
            ...
            if (this.getMenuPrincipal() != null) {
                if (this.getMenuPrincipal().getMostrar()) {
                    ...
                    //Muestra el sonido pertinetne si asi se ha establecido:
                    if (this.getSonidoActivado()) {
                        MotorSonido.menuOpcionCambiada();
                    }
    ...
    protected void cicloJuego() {
        ...
        if (this.juegoPausado) {
            representarTodo(this.getContenedorGrafico());
            return;
        } //Antes de salir hace un repintado por si es necesario (seria mejor buscar otro metodo mas eficiente).
        ...
        } else //else innecesario.
        {
                ...
                if (piezaActual.calcularColision(this.getTablero())) {
                    ...
                    if (lineasHechas > 0) {
                        ...
                        if (!this.menuPrincipal.getMostrar()) {
                            this.ignorarTeclas = true;
                        } //Si no esta en el menu principal, se ignoran las teclas mientras haya animacion.
                        MotorAnimaciones.hacerLineas(this.getContenedorGrafico(), this.imagenBuffer, this.getTablero(), filasLinea);
                        if (!this.menuPrincipal.getMostrar()) {
                            this.ignorarTeclas = false;
                        }
            ...
            if (this.piezaActual == null || sacarPieza) {
                this.sacarPieza();
            }
