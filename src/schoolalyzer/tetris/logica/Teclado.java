...
public class Teclado implements KeyListener {
    ...
    static public boolean esTeclaRotacion(int tecla) {
        ...
        if (TetrisMain.game != null) {
            if (TetrisMain.game.teclaEnTeclas(tecla, TetrisMain.game.getTeclaRotarDerecha()) || TetrisMain.game.teclaEnTeclas(tecla, TetrisMain.game.getTeclaRotarIzquierda())) {
                loEs = true;
            }
        }
    ...
    static synchronized public void procesarTecla(int tecla, boolean contarTiempo) {
        if (TetrisMain.game != null) {
            ...
            long tiempoNecesario = TetrisMain.game.getRetrasoTeclasOtras();
            if (Teclado.esTeclaRotacion(tecla)) {
                tiempoNecesario = TetrisMain.game.getRetrasoTeclasRotacion();
            }
            long tiempoActual = System.currentTimeMillis();
            long tiempoTranscurrido = tiempoActual - Teclado.ultimaLecturaTeclaOtras;
            if (Teclado.esTeclaRotacion(tecla)) {
                tiempoTranscurrido = tiempoActual - Teclado.ultimaLecturaTeclaRotacion;
            }
            ...
            //Procesa la tecla:
            TetrisMain.game.procesarTecla(tecla);

            //Si se ha enviado incrementar el contador de la ultima lectura, se procede:
            if (contarTiempo) {
                if (Teclado.esTeclaRotacion(tecla)) {
                    Teclado.ultimaLecturaTeclaRotacion = System.currentTimeMillis();
                } else {
                    Teclado.ultimaLecturaTeclaOtras = System.currentTimeMillis();
                }
    ...
    static synchronized public void leerTeclado(boolean limitadorTiempo) {
        ...
        while (teclas.hasMoreElements()) {
            ...
            if (pulsada && TetrisMain.game.teclaEnTeclasTodas(tecla)) {

                //Si se ha enviado utilizar el limitador de tiempo:
                if (limitadorTiempo) {
                    //Si aun no ha pasado el tiempo necesario, sale:
                    tiempoNecesario = TetrisMain.game.getRetrasoTeclasOtras();
                    if (Teclado.esTeclaRotacion(tecla)) {
                        tiempoNecesario = TetrisMain.game.getRetrasoTeclasRotacion(); /*System.out.println("tekla rotacion!!!");*/ }
                ...
                if (Teclado.esTeclaRotacion(tecla)) {
                    seHaProcesadoAlgunaTeclaRotacion = true;
                } else {
                    seHaProcesadoAlgunaTeclaOtras = true;
                }
        ...
        if (seHaProcesadoAlgunaTeclaOtras) {
            Teclado.ultimaLecturaTeclaOtras = System.currentTimeMillis();
        }
        if (seHaProcesadoAlgunaTeclaRotacion) {
            Teclado.ultimaLecturaTeclaRotacion = System.currentTimeMillis();
        }
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (TetrisMain.game != null) {
            ...
            if (Teclado.teclasPulsadas.get(tecla) != null && Teclado.teclasPulsadas.get(tecla)) {
                return;
            }

            //Si la tecla es una tecla valida para el juego:
            if (TetrisMain.game.teclaEnTeclasTodas(tecla)) {
                ...
                int[] teclasIzquierda = TetrisMain.game.getTeclaIzquierda();
                int[] teclasDerecha = TetrisMain.game.getTeclaDerecha();
                int[] teclasArriba = TetrisMain.game.getTeclaArriba();
                int[] teclasAbajo = TetrisMain.game.getTeclaAbajo();

                if (TetrisMain.game.teclaEnTeclas(tecla, teclasIzquierda)) //Si se ha apretado izquierda, desactiva derecha.
                {
                ...
                } else if (TetrisMain.game.teclaEnTeclas(tecla, teclasDerecha)) //Si se ha apretado derecha, desactiva izquierda.
                {
                    //Desactiva todos los codigos que hagan la misma accion:
                    for (int x = 0; x < teclasIzquierda.length; x++) {
                        Teclado.teclasPulsadas.put(teclasIzquierda[x], false);
                    }
                } else if (TetrisMain.game.teclaEnTeclas(tecla, teclasArriba)) //Si se ha apretado arriba, desactiva abajo.
                {
                    //Desactiva todos los codigos que hagan la misma accion:
                    for (int x = 0; x < teclasAbajo.length; x++) {
                        Teclado.teclasPulsadas.put(teclasAbajo[x], false);
                    }
                } else if (TetrisMain.game.teclaEnTeclas(tecla, teclasAbajo)) //Si se ha apretado abajo, desactiva arriba.
                {
                    //Desactiva todos los codigos que hagan la misma accion:
    ...
    public void keyReleased(KeyEvent e) {
        ...
        if (TetrisMain.game != null) {
            //Si la tecla es una tecla valida para el juego:
            if (TetrisMain.game.teclaEnTeclasTodas(e.getKeyCode())) {
