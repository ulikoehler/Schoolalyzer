...
import schoolalyzer.tetris.logica.Menu;
//import logica.Juego;
import schoolalyzer.tetris.logica.Panel;
import schoolalyzer.tetris.logica.Pieza;
import schoolalyzer.tetris.logica.Tablero;
...
public class MotorGrafico {
    ...
    static protected final int numeroCodificacionPiezas = schoolalyzer.tetris.configuracion.Otros.getNumeroCodificacionPiezas();
    static protected final int VACIO = schoolalyzer.tetris.configuracion.Otros.getNumeroVacio(); //Numero que nos marca un vacio.
    static protected boolean usarImagenes = schoolalyzer.tetris.configuracion.Aspecto.getUsarImagenes();
    static protected byte celdaAncho = schoolalyzer.tetris.configuracion.Aspecto.getCeldaAncho();
    static protected byte celdaAlto = schoolalyzer.tetris.configuracion.Aspecto.getCeldaAlto();
    static protected Image[] imagenes = new Image[Byte.MAX_VALUE];
    static protected String directorioImagenes = schoolalyzer.tetris.configuracion.Otros.getDirectorioImagenes();
    ...
    static public void dibujar(Graphics g, Object objeto) {
        if (objeto instanceof Pieza) {
            dibujarPieza(g, (Pieza) objeto);
        } else if (objeto instanceof Tablero) {
            dibujarTablero(g, (Tablero) objeto);
        } else if (objeto instanceof Panel) {
            dibujarPanel(g, (Panel) objeto);
        } else if (objeto instanceof Menu) {
            dibujarMenu(g, (Menu) objeto);
        }
    }
    ...
    static protected void dibujarPanelInformacion(Graphics g, Panel panel) {
        ...
        int y = (panel.getTop() + panel.getAlto() + panel.getMargen()) * MotorGrafico.getCeldaAlto() + schoolalyzer.tetris.configuracion.Aspecto.getEspacioLineasTexto();

        //Borra el texto anterior, por si quedaba en pantalla:
        g.clearRect(x, y - schoolalyzer.tetris.configuracion.Aspecto.getEspacioLineasTexto(), 400, schoolalyzer.tetris.configuracion.Aspecto.getEspacioLineasTexto() * 5);

        //Pone el texto:
        Color colorTexto = panel.getPaleta().getColor(panel.getColorTexto());
        Font fuenteTexto = new Font(Font.SANS_SERIF, Font.PLAIN, schoolalyzer.tetris.configuracion.Aspecto.getDimensionTexto());
        g.setColor(colorTexto);
        g.setFont(fuenteTexto);
        x += schoolalyzer.tetris.configuracion.Aspecto.getDimensionTexto();
        MotorGrafico.dibujarTexto(g, schoolalyzer.tetris.configuracion.Idiomas.nivel[schoolalyzer.tetris.principal.TetrisMain.getIdiomaActual()] + ": " + nivel, x, y, schoolalyzer.tetris.configuracion.Aspecto.getEspacioLineasTexto());
        y += schoolalyzer.tetris.configuracion.Aspecto.getEspacioLineasTexto();
        MotorGrafico.dibujarTexto(g, schoolalyzer.tetris.configuracion.Idiomas.puntuacion[schoolalyzer.tetris.principal.TetrisMain.getIdiomaActual()] + ": " + puntos, x, y, schoolalyzer.tetris.configuracion.Aspecto.getEspacioLineasTexto());
        y += schoolalyzer.tetris.configuracion.Aspecto.getEspacioLineasTexto();
        MotorGrafico.dibujarTexto(g, schoolalyzer.tetris.configuracion.Idiomas.lineasTotales[schoolalyzer.tetris.principal.TetrisMain.getIdiomaActual()] + ": " + lineas, x, y, schoolalyzer.tetris.configuracion.Aspecto.getEspacioLineasTexto());
        y += schoolalyzer.tetris.configuracion.Aspecto.getEspacioLineasTexto();
        MotorGrafico.dibujarTexto(g, schoolalyzer.tetris.configuracion.Idiomas.lineasNivel[schoolalyzer.tetris.principal.TetrisMain.getIdiomaActual()] + ": " + lineasNivel, x, y, schoolalyzer.tetris.configuracion.Aspecto.getEspacioLineasTexto());
        y += schoolalyzer.tetris.configuracion.Aspecto.getEspacioLineasTexto();
    }
    ...
    static public void dibujarMenu(Graphics g, Menu menu) {
        ...
        MotorGrafico.dibujarTexto(g, menu.getTextoFijo(), menu.getTextoFijoLeft(), menu.getTextoFijoTop(), schoolalyzer.tetris.configuracion.Aspecto.getEspacioLineasTexto());
        ...
        for (int x = 0; x < opciones.length; x++) {
            ...
            MotorGrafico.dibujarTexto(g, opciones[x], left, top, schoolalyzer.tetris.configuracion.Aspecto.getEspacioLineasTexto());
            top += schoolalyzer.tetris.configuracion.Aspecto.getEspacioLineasTexto();
        }
        ...
        if (menu.getTextoAlternativoActual() != null) {
            if (menu.getTextoAlternativoActual().trim() != "") {
                ...
                MotorGrafico.dibujarTexto(g, menu.getTextoAlternativoActual(), left, top, schoolalyzer.tetris.configuracion.Aspecto.getEspacioLineasTexto());
            }
        }
    }
    ...
    static public void dibujarTexto(Graphics g, String texto, int left, int top, int espacioEntreLineas) {
        ...
        while (texto.trim().length() > 0) {
            if (texto.indexOf("\n") != -1) {
                textoParcial = texto.substring(0, texto.indexOf("\n"));
            } else {
                textoParcial = texto;
            }
            if (texto.indexOf("\n") != -1) {
    ...
    static public void dibujarGraficoBackup(Graphics g, Paleta paleta, int[][] matriz, byte left, byte top, byte ancho, byte alto, boolean dibujarVacios) {
        ...
        for (int f = 0; f < alto; f++) {
            for (int c = 0; c < ancho; c++) {
                if (MotorGrafico.graficoBackupPintado[f][c]) {
                    graficoBackup[f][c] = matriz[f][c];
                } else {
                    graficoBackup[f][c] = MotorGrafico.VACIO;
                }
            }
        }
    ...
    static synchronized public void dibujarMatriz(Graphics g, Paleta paleta, int[][] matriz, byte left, byte top, byte ancho, byte alto, boolean dibujarVacios) {
        ...
        if (!MotorGrafico.graficoInicializado) {
            MotorGrafico.inicializarGraficoBackup();
            MotorGrafico.graficoInicializado = true;
        }
        ...
        for (byte f = 0; f < alto; f++) {
            ...
            for (byte c = 0; c < ancho; c++) {
                ...
                if (indiceColor == MotorGrafico.VACIO && !dibujarVacios) {
                    x++;
                    continue;
                }
    ...
    static synchronized protected void dibujarCasilla(Graphics g, Paleta paleta, int indiceColor, byte x, byte y) //////////
    {
        ...
        if (indiceColor < 0) {
            indiceColor /= MotorGrafico.numeroCodificacionPiezas;
        }

        //Si la casilla que se va a pintar es igual a la que ya hay (y ua se ha pintado), no la pinta:
        if (MotorGrafico.graficoBackup[y][x] == indiceColor && graficoBackupPintado[y][x]) {
            return;
        } //<-- parece ser ke no va porke el metodo paint lo borra todo y deja el background!!!
        else {
            MotorGrafico.actualizarGraficoBackup(indiceColor, x, y);
        } //Actualiza el grafico backup, para la proxima vez.
        ...
        if (MotorGrafico.getUsarImagenes()) {
            ...
            if (MotorGrafico.imagenes[indiceColor] == null) {
            ...
            if (img != null) {
                ...
                if (schoolalyzer.tetris.configuracion.Otros.getTipoPrograma() == 0) {
                    tracker = new MediaTracker(TetrisMain.ventanaPrincipal);
                } else if (schoolalyzer.tetris.configuracion.Otros.getTipoPrograma() == 1) {
                    tracker = new MediaTracker(TetrisMain.appletPrincipal);
                }
    ...
    static public void limpiarImagenBuffer(BufferedImage imagenBuffer, Paleta paleta) {
        ...
        contenedorBuffer.setColor(schoolalyzer.tetris.configuracion.Aspecto.getColorFondoImagenBuffer());
        ...
        if (schoolalyzer.tetris.configuracion.Aspecto.getUsarImagenFondo()) {
        ...
        if (ancho > Byte.MAX_VALUE) {
            ancho = Byte.MAX_VALUE;
        }
        if (alto > Byte.MAX_VALUE) {
            alto = Byte.MAX_VALUE;
        }
    ...
    static public String toString(Object o) {
        ...
        if (TetrisMain.game != null) {
            filas = (byte) (TetrisMain.game.getTablero().getAlto());
            columnas = (byte) (TetrisMain.game.getPanel().getLeft() + TetrisMain.game.getPanel().getAncho());
        }
